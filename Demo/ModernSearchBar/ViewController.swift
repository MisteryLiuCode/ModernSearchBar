...
//        self.configureSearchBarWithUrl()
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        print("输入发生改变了")
        configureSearchBar()
        requesetData(keyWord: searchText)
    }
...
extension ViewController {
    func requesetData(keyWord: String) {
        //发送get请求,带中文
        let requestUrl = "http://101.43.145.108:49032/inputPrompt"
        let parameters = [ "oriLong": "116.38974090824217","oriLat":"39.9364888775626","keywords":keyWord]
        //发送请求，获取目的地，中文
//        AF.request("http://101.43.145.108:49032/inputPrompt/116.38974090824217/39.9364888775626/\(keyWord)")
        AF.request(requestUrl,parameters: parameters).responseJSON { (response) in
            ...
            DispatchQueue.main.async {
                ...
                case.success(let json):
                    ...
                    let arr = try! JSONDecoder().decode([String].self, from: response.data ?? Data())
                    print(arr)
                    self.modernSearchBar.setDatas(datas: arr)
